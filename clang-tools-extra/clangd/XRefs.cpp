...
std::vector<DocumentLink> getDocumentLinks(ParsedAST &AST) {
  ...
  for (auto &Inc : AST.getIncludeStructure().MainFileIncludes) {
    ...
    // get the # Token itself, std::next to get the "include" token and the first token after (aka "File Token")
    ...
    // "string_literal" if using the "#include "q-char-sequence" new-line" syntax
    // something else (most likely "identifier") if using the "#include pp-tokens new-line" syntax (#include with macro argument)

    CharSourceRange IncArgRange;

    if(FileTok->kind() == tok::TokenKind::less)
    {
      // FileTok->range would only include the '<' char. Hence we explicitly use Inc.Written's length.
      IncArgRange = syntax::FileRange(SM, FileTok->location(), Inc.Written.length()).toCharRange(SM);
    }
    else if(FileTok->kind() == tok::TokenKind::string_literal)
    {
      // FileTok->range includes the quotes for string literals so just return it.
      IncArgRange = FileTok->range(SM).toCharRange(SM);
    }
    else
    {
      // FileTok is the first Token of a macro spelling
      // We can use the AST to get the macro expansion from the spelling starting at FileTok
      // and use the expansion to get all the spelled Tokens that expanded to it

      auto OptExpansion = AST.getTokens().expansionStartingAt(FileTok);
      if(OptExpansion && !OptExpansion->Spelled.empty())
      {
        // If an expansion was found and has an non-empty spelling, return the range from the start
        // of the first Token to the end of the last Token
        const auto & LastTok = OptExpansion->Spelled.back();

        IncArgRange = FileTok->range(SM).toCharRange(SM);
        const auto EndRange = LastTok.range(SM).toCharRange(SM);
        IncArgRange.setEnd(EndRange.getEnd());
      }
      else
      {
    ...
    Result.push_back(
        DocumentLink({halfOpenToRange(SM, IncArgRange),
